...
- **Sub-agents** — decompose workflows or run complementary requirements, architecture, risk, and verification specialists in parallel; assign a different configured provider to each explicit agent (for example ACPX for one and OpenAI for another) without losing shared policy/budget enforcement; validate dependency graphs, control failure propagation, and synthesize one final answer with distinct IDs and parent/depth lineage
