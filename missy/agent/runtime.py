...
class AgentConfig:
    ...
    system_prompt: str = (
        ...
        "own provider registry and real configured credentials (vault/OAuth/"
        "config api_key). NEVER shell out to `openai`/`ollama` CLI binaries "
        "or check environment variables like OPENAI_API_KEY yourself — those "
        "are unrelated to how Missy actually authenticates its providers "
        "(see `missy providers list`) and will report a false failure even "
        "when the provider works fine. "
...
DISCORD_SYSTEM_PROMPT = (
    ...
    "— it uses Missy's own provider registry and real configured "
    "credentials (vault/OAuth/config api_key). NEVER shell out to "
    "`openai`/`ollama` CLI binaries or check environment variables like "
    "OPENAI_API_KEY yourself — those are unrelated to how Missy actually "
    "authenticates its providers and will report a false failure even when "
    "the provider works fine. "
    ...
    "pings anyone, it just renders as dead text. Every message you receive "
    "tells you the sender's real Discord user ID, and lists other users "
    "recently active in the channel with their real IDs too — use one of "
    "those IDs directly. If the person you need to tag is NOT listed there "
    "(they haven't spoken recently), call discord_lookup_user(guild_id=..., "
    "query=<their name>) first to find their real ID, then use <@THEIR_ID> "
    "in your reply. Never invent, guess, or reuse an ID from a different "
    "conversation — an ID that doesn't belong to a real member of the "
    "current channel simply won't ping. "
