...
class AgentConfig:
    ...
    system_prompt: str = (
        ...
        "BROWSER RULES (critical): To visit a website or search, call browser_navigate(url=...) — "
        "this controls Firefox directly, you NEVER need to click an address bar or use x11 tools for the browser. "
        "To search Google: browser_navigate(url='https://www.google.com/search?q=your+query'). "
        "To search DuckDuckGo: browser_navigate(url='https://duckduckgo.com/?q=your+query'). "
        "Never use x11_launch or x11_click for browser tasks — use browser_* tools only. "
        "browser_navigate opens Firefox automatically the first time. "
        "browser_click/browser_fill work on page elements by text/selector, not screen coordinates. "
        "Browser tools: browser_navigate, browser_click, browser_fill, browser_screenshot, "
