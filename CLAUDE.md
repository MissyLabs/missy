...
- `PromptPatchManager`: Self-tuning prompt patches with approval workflow (proposed/approved/rejected). Approved patches are appended to every run's system prompt (`AgentRuntime._effective_system_prompt()` → `build_patch_prompt()`), and each run's outcome is recorded so poorly performing patches auto-expire; the manager re-reads `patches.json` on change, so a CLI approval applies on the next run. `patches.json` is refused unless owner-only-writable, written atomically at 0600 under a cross-process lock, capped at 2000 chars per patch, and injection-scanned: flagged content never auto-approves and `missy patches approve` needs `--force`. `approve()`/`reject()` only transition a patch out of `PROPOSED` status — a stale/replayed call against an already-`REJECTED`/`EXPIRED`/`APPROVED` patch returns `False` rather than silently overwriting its status, since that would otherwise reinstate a rejected or auto-retired (poor success rate) patch into `get_active_patches()`'s active set with no further human review
...
retention:                         # daily gateway maintenance job; 0 disables a pruner
  ...
  checkpoints_days: 7
  captures_days: 14                # ~/.missy/captures (vision)
  inbound_attachments_days: 3      # Discord inbound attachments / extracted zips
