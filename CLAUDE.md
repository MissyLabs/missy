...
**Video Generation (`missy/tools/builtin/video_generate.py`)** — `VideoGenerateTool` (agent tool name `video_generate`) talks to a separately-running [ComfyUI](https://github.com/comfyanonymous/ComfyUI) server's HTTP API (default `127.0.0.1:8199`) to render short video clips, optionally with a soundtrack, in three backends selected via a `backend` parameter (audit/design record in `video.md` at the repo root):
...
**Audio**: `audio_prompt` generates a soundtrack (music/sfx/ambience) with Stable Audio Open 1.0 inside the same workflow graph, sized to the final clip duration, and muxes it into the MP4 via `VHS_VideoCombine`'s `audio` input; `audio_path` muxes an existing local audio file instead (mutually exclusive). Requires `stable-audio-open-1.0.safetensors` (`models/checkpoints/`) + `t5_base.safetensors` (`models/text_encoders/`, loaded via `CLIPLoader(type="stable_audio")` — the checkpoint embeds no text encoder).
