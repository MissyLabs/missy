...
CLI (missy/cli/main.py)
  ...
  → AgentRuntime (agent/runtime.py)
       ...
       ├─ SQLiteMemoryStore (memory/) — ResilientMemoryStore wrapper exists but is not wired in
...
- `VoiceChannel`: WebSocket server (default port 8765) accepting connections from edge nodes (ReSpeaker, Raspberry Pi). Protocol: JSON control frames + binary PCM audio. Device pairing with PBKDF2-hashed tokens. Unauthenticated `pair_request` frames are validated (short plain `friendly_name`/`room` labels that pass the injection scanner, small flat `hardware_profile`) and limited to 3 per source IP per hour and 20 pending total; network-originated nodes are created `safe-chat`, `missy devices pair --mode full` grants full access, and unapproved requests expire via the retention job. Per-node policy modes: `full`, `safe-chat`, `muted`. `VoiceServer._message_loop()` re-fetches the node's live policy from the registry on every message (not just at the initial auth handshake), so `missy devices policy <id> --mode muted` disconnects an already-connected node on its next message rather than only blocking future connection attempts. `missy devices policy <id> --mode safe-chat` routes that node's agent calls to a dedicated `capability_mode="safe-chat"` `AgentRuntime` (`gateway_start()` constructs one alongside the main/Discord runtimes) via `_build_agent_callback()`'s `policy_mode`-based dispatch — a safe-chat node's request is refused outright (fail closed) rather than silently served with full access if no restricted runtime is configured. STT via faster-whisper, TTS via piper binary.
...
- `PromptPatchManager`: Self-tuning prompt patches with approval workflow (proposed/approved/rejected). Approved patches are appended to every run's system prompt (`AgentRuntime._effective_system_prompt()` → `build_patch_prompt()`), and each run's outcome is recorded so poorly performing patches auto-expire; the manager re-reads `patches.json` on change, so a CLI approval applies on the next run. `patches.json` is refused unless owner-only-writable (a refused store is never written -- mutations raise `PatchStoreRefusedError` instead of replacing it -- and a `chmod 600` fix is picked up without a restart), written atomically at 0600 under a cross-process lock, capped at 2000 chars per patch, and injection-scanned: flagged content never auto-approves and `missy patches approve` needs `--force`. `approve()`/`reject()` only transition a patch out of `PROPOSED` status — a stale/replayed call against an already-`REJECTED`/`EXPIRED`/`APPROVED` patch returns `False` rather than silently overwriting its status, since that would otherwise reinstate a rejected or auto-retired (poor success rate) patch into `get_active_patches()`'s active set with no further human review
...
- `CostTracker`: Per-session cost tracking and budget enforcement (`max_spend_usd`). The process-wide `GlobalBudget` (`global_max_spend_usd`) is installed by every entry point (`ask`/`run`/`recover`/`api start`/`gateway start`, the latter before proactive triggers and the scheduler, and again on hot reload); a corrupt `~/.missy/global_budget.json` fails closed (quarantined to `.corrupt`, period treated as fully spent until `missy budget reset`). Raises `BudgetExceededError` when cap hit. Each session's tracker is created lazily (`AgentRuntime._make_cost_tracker()`) from `self.config.max_spend_usd` at first use; `gateway_start()`'s config hot-reload callback mutates `_agent.config.max_spend_usd`/`_discord_agent.config.max_spend_usd`/the proactive-trigger runtime's config/`SchedulerManager._default_max_spend_usd` in place on every reload (alongside the `PolicyEngine`/`ProviderRegistry`/`OtelExporter`/`AuditLogger` re-init `_apply_config()` already does) — without this, editing `max_spend_usd` in `config.yaml` while the gateway keeps running had zero effect on any of these long-lived, already-constructed runtimes (not even for brand-new sessions) until a full restart, since none of them are rebuilt on reload the way the other singletons are.
...
- `SleeptimeWorker`: Opt-in background daemon thread (`sleeptime.enabled`, default `false`; `AgentConfig.sleeptime_enabled` is only set for the gateway's main runtime). LLM summaries run only with an explicit `sleeptime.provider`; with none, summaries are local/deterministic and make no provider calls (see `issue.md` for the quota incident that drove this). One owner per memory store is enforced both in-process and across processes (non-blocking `flock` on `<memory.db>.sleeptime.lock`; a refused start is audited as `sleeptime.start_refused`). Wakes every `check_interval_seconds` (default 60s); once the agent has been idle for `idle_threshold_seconds` (default 300s, reset via `record_activity()` on every `run()`/`run_stream()`/`resume_checkpoint()` call) it summarizes unsummarised turns and extracts learnings for eligible sessions (inspired by Letta sleeptime computing). `AgentRuntime.shutdown()` stops it cleanly.
...
**Memory (`missy/memory/`)** — `SQLiteMemoryStore` at `~/.missy/memory.db` with FTS5 search. Stores conversation turns and learnings. `cleanup()` removes turns older than N days (and, with `include_summaries=True`, summaries whose source turns are all gone); every turn-deleting path also removes the affected `large_content` rows and refreshes `sessions.turn_count` in the same transaction, and `missy doctor` runs the FTS5 integrity check. `ResilientMemoryStore` (in-memory fallback wrapper, LRU-bounded cache) is available but not used by `AgentRuntime`. Optional `VectorMemoryStore` with FAISS semantic search (`pip install -e ".[vector]"`). `GraphMemoryStore` provides SQLite-backed entity-relationship graph memory with rule-based pattern matching for structured knowledge retrieval.
...
scheduling:
  ...
  active_hours: ""                 # e.g. "08:00-22:00"; applies to jobs without their own window
  misfire_grace_seconds: 300       # a run missed by up to this long (e.g. restart) still fires once

features:                          # opt-in agent features, applied to every AgentConfig
  graph_memory_enabled: false
  semantic_memory_enabled: false
  prompt_patch_proposals_enabled: false
  model_routing_enabled: false
  condenser_pipeline_enabled: false
  condenser_min_messages: 30

retention:                         # daily gateway maintenance job; 0 disables a pruner
  enabled: true
  # Pruners that delete operator content are opt-in (default 0); the gateway
  # logs a preview of what an enabled file pruner will delete at startup.
  memory_days: 0                   # conversation turns (pinned kept) + orphaned summaries
  captures_days: 0                 # ~/.missy/captures (vision)
  inbound_attachments_days: 0      # Discord inbound attachments / extracted zips
  graph_memory_days: 0
  # Internal bookkeeping, pruned by default:
  checkpoints_days: 7
  request_tracker_days: 30
  pending_pairing_hours: 24        # unapproved voice pairing requests

global_max_spend_usd: 0.0           # process-wide cap across sessions (0 = off); installed by every entry point
global_budget_period: total         # total | daily | monthly
...
missy patches approve PATCH_ID      Approve a proposed patch (--force if the injection scanner flags it)
...
missy devices pair                  Approve a pending pairing request (--node-id, --mode full|safe-chat|muted)
