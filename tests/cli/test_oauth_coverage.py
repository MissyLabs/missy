...
class TestStartCallbackServer:
    def test_returns_server_on_success(self):
        ...
        with patch("missy.cli.oauth.HTTPServer", return_value=mock_server):
            with patch("missy.cli.oauth.threading.Thread", return_value=mock_thread):
...
class TestRefreshTokenIfNeededStalePath:
    ...
    def test_returns_stale_token_when_no_client_id(self):
        ...
        with patch("missy.cli.oauth.load_token", return_value=token_data):
            with patch("missy.cli.oauth.DEFAULT_CLIENT_ID", ""):
                result = __import__(
                    ...
                    with patch("missy.cli.oauth._callback_result") as mock_result:
                        ...
                        with patch("missy.cli.oauth.console") as mock_console:
                            ...
                            with patch("missy.cli.oauth.threading.Event") as MockEvent:
                                ...
                                with patch("missy.cli.oauth.threading.Thread"):
                                    # No code → aborts
                                    with patch(
                                        "missy.cli.oauth._extract_code_from_url", return_value=None
                                    ):
...
class TestPromptThreadExceptionHandler:
    ...
    def test_run_oauth_prompt_thread_handles_exception(self):
        ...
        with patch("missy.cli.oauth._start_callback_server", return_value=None):
            with patch("missy.cli.oauth.webbrowser.open"):
                with patch("missy.cli.oauth._callback_event"):
                    with patch("missy.cli.oauth._callback_result"):
                        with patch(
...
class TestAutomaticCallbackWins:
    def test_automatic_callback_code_used_over_paste(self):
        ...
        with patch("missy.cli.oauth._start_callback_server", return_value=mock_server):
            with patch("missy.cli.oauth.webbrowser.open"):
