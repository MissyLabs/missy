...
class TestVisionShutdown:
    ...
    def test_shutdown_returns_summary(self):
        ...
        with (
            patch("missy.vision.scene_memory.get_scene_manager") as mock_sm,
    ...
    def test_shutdown_scene_manager_exception(self):
        ...
        with (
            patch("missy.vision.scene_memory.get_scene_manager", side_effect=RuntimeError("fail")),
            patch("missy.vision.health_monitor.get_health_monitor") as mock_hm,
            patch("missy.vision.audit.audit_vision_session"),
        ):
            mock_hm.return_value._persist_path = None
    ...
    def test_shutdown_health_monitor_exception(self):
        ...
        with (
            ...
            patch(
                ...
                side_effect=RuntimeError("hm fail"),
            ),
            patch("missy.vision.audit.audit_vision_session"),
        ):
            mock_sm.return_value.list_sessions.return_value = []
...
class TestVisionMemoryBridge:
    ...
    def test_get_session_context_with_observations(self):
        ...
        bridge.recall_observations = MagicMock(
            return_value=[
                {
                    "task_type": "puzzle",
                    "observation": "Found edge",
                    "confidence": 0.9,
                    "timestamp": "T1",
                },
            ]
...
class TestConfigValidatorEdgeCases:
    ...
    def test_threshold_below_zero(self):
        ...
        assert any(
            i.field == "auto_activate_threshold" and i.severity == "error" for i in result.issues
        )
    ...
    def test_threshold_low_warning(self):
        ...
        assert any(
            i.field == "auto_activate_threshold" and i.severity == "warning" for i in result.issues
        )
    ...
    def test_scene_frames_zero(self):
        ...
        assert any(
            i.field == "scene_memory_max_frames" and i.severity == "error" for i in result.issues
        )
    ...
    def test_scene_frames_excessive(self):
        ...
        assert any(
            i.field == "scene_memory_max_frames" and i.severity == "warning" for i in result.issues
        )
    ...
    def test_scene_sessions_zero(self):
        ...
        assert any(
            i.field == "scene_memory_max_sessions" and i.severity == "error" for i in result.issues
        )
    ...
    def test_scene_sessions_excessive(self):
        ...
        assert any(
            i.field == "scene_memory_max_sessions" and i.severity == "warning"
    ...
    def test_all_valid_resolutions(self):
        ...
        for w, h in [(640, 480), (1280, 720), (1920, 1080), (3840, 2160)]:
            ...
            assert not any(
                i.field == "capture_width" and i.severity == "warning" for i in result.issues
