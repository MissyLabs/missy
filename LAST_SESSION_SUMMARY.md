...
## Changed (23 checkpoints this session, full suite green after every one)
...
### Seventeen independent, confirmed critical security vulnerabilities (full detail in AUDIT_SECURITY.md)
...
- **#9** SR-1.x through SR-4.x security review remediation — this
  ...
  SR-1.9a, SR-1.10, SR-1.11, SR-1.12, SR-1.13, SR-2.3, SR-2.4, SR-3.2,
  SR-3.3, SR-3.4 (plus SR-3.1 substantially via FX-B). Remaining: SR-1.1
  ...
  rather than re-resolving at connect time), SR-2.1/SR-2.2 (scheduled
  jobs default to full capability mode; proactive triggers default to
  no-confirmation with no gate wired — both are product-policy
  default-value questions more than mechanical bugs), SR-3.5
  (non-atomic JSON store writes — flagged as likely-already-resolved-by-
  FX-B, tracked as task #30; treat that flag with real skepticism —
  SR-3.3's checkpoint was flagged the same way and turned out to be two
  stacked, fully live, never-worked-at-all bugs instead), the
...
- **#17** FX-G process-group cleanup on acpx timeout — implementation
  ...
  session.
- **#30** SR-3.5 (atomic concurrent persistence) — flagged as
  likely-already-resolved-by-FX-B, but apply real skepticism to that
  flag: verify against current code, don't assume (see SR-3.3 lesson
  below).
...
everything else. Otherwise, §3 now has three of five items done
(SR-3.4's ordering defect, SR-3.2's Summarizer fix, SR-3.3's memory-tool
dispatch wiring); SR-3.5 (task #30) is the natural next candidate — it's
flagged as likely-already-fixed-by-FX-B, but SR-3.3 was flagged
identically and turned out to be two fully live, never-worked-at-all
bugs, so treat that flag as an open question, not a shortcut. Apply the
same discipline: read the actual current code, trace actual runtime
call paths, and specifically check whether any existing test dispatches
through the real production entry point (not just the unit under test
in isolation) before concluding anything is fixed. Live-reproduce
before declaring anything fixed or broken either way. After that, §2 is
down to SR-2.1/SR-2.2, both genuine product-policy
decisions (should `capability_mode` default to something narrower than
`"full"` for scheduled jobs? should `requires_confirmation` default to
`True`, and should the production CLI construct/inject a real
`ApprovalGate`?) rather than mechanical bug fixes — worth flagging for
explicit product input before changing defaults that could break
