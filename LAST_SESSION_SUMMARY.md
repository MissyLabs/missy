...
## Changed (22 checkpoints this session, full suite green after every one)
...
### Sixteen independent, confirmed critical security vulnerabilities (full detail in AUDIT_SECURITY.md)
...
- **#9** SR-1.x through SR-4.x security review remediation — this
  ...
  SR-3.4 (plus SR-3.1 substantially via FX-B). Remaining: SR-1.1 (audit
  ...
  default-value questions more than mechanical bugs), SR-3.3/SR-3.5
  (large-tool-output retrieval, non-atomic JSON store writes — both
  flagged as likely-already-resolved-by-FX-B, tracked as tasks #29/#30,
  need the same verify-before-fixing checkpoint SR-3.2 just got), the
...
- **#17** FX-G process-group cleanup on acpx timeout — implementation
  ...
  session.
- **#29/#30** SR-3.3/SR-3.5 — new this checkpoint, likely quick
  verification-only checkpoints (see above).
- **Lesson worth remembering for future test-double changes** (from
  this checkpoint): a bare `MagicMock()` with no `spec` auto-vivifies
  any attribute access, so a test that mocks a provider/client and calls
  a method name that doesn't exist on the real interface will silently
  pass forever instead of catching the typo — always construct
  interface-heavy mocks with `spec=<the real class>` so a renamed or
  nonexistent method call fails the test the same way it would fail in
  production.
...
everything else. Otherwise, §3 now has two of five items done (SR-3.4's
ordering defect, SR-3.2's Summarizer fix); SR-3.3 and SR-3.5 (tasks
#29/#30) are natural next candidates precisely *because* they're
flagged as likely-already-fixed-by-FX-B and may turn out to be
quick verification-only checkpoints rather than new fixes — apply the
same discipline SR-3.2 used: read the actual current code and trace
actual runtime call paths before assuming the review's text is still
accurate, live-reproduce before declaring anything fixed or broken.
After that, §2 is down to SR-2.1/SR-2.2, both genuine product-policy
